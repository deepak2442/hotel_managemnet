...
export function DailyReport() {
  ...
  const handleGenerate = async () => {
    const data = await getDailyReport(new Date(selectedDate));
    setReportData(data);
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(
      ...
      `daily-report-${selectedDate}`,
      {
        ...
        dateRange: format(new Date(selectedDate), 'MMM dd, yyyy'),
      }
    );
  };
  ...
  const handlePrint = () => {
    ...
    printReport('printable-report', `Daily Report - ${format(new Date(selectedDate), 'MMMM dd, yyyy')}`);
  };
        ...
        <div id="printable-report" className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-6 no-print">
            <h3 className="text-xl font-semibold text-gray-900">
              Daily Report - {format(new Date(reportData.date), 'MMMM dd, yyyy')}
            </h3>
